...
import { managerNativeVaultTrackedHeadState } from './manager-native-vault-source.mjs';
...
export function verifyManagerNativeBoundaryApk(apkPath) {
  ...
  if (proof.source_commit_exact) {
    const sourceState = managerNativeVaultTrackedHeadState(pluginRoot, {
      repositoryRoot,
      revision: proof.source_commit,
    });
    if (!sourceState.tracked_head_exact) {
      throw new Error('Manager APK claims commit-exact provenance but native source inventory differs from that tracked commit');
    }
  }
  return {
    ok: true,

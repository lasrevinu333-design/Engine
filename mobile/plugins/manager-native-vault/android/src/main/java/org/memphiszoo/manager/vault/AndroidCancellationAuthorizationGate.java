...
final class AndroidCancellationAuthorizationGate implements CancellationAuthorizationGate {
    ...
    public boolean confirm(String operationId, String deviceId) throws VaultFailure {
        if (!promptActive.compareAndSet(false, true)) throw new VaultFailure("manager_native_cancellation_confirmation_busy");
        Activity activity = activitySupplier.get();
        if (activity == null || activity.isFinishing() || activity.isDestroyed()) {
            promptActive.set(false);
            throw new VaultFailure("manager_native_cancellation_confirmation_unavailable");
        }
        ...
        try {
            if (!ready.await(TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
                ...
                throw new VaultFailure("manager_native_cancellation_confirmation_timeout");
            }
            return Boolean.TRUE.equals(decision.get());
        } catch (InterruptedException error) {
            Thread.currentThread().interrupt();
            throw new VaultFailure("manager_native_cancellation_confirmation_interrupted", error);
        } finally {
            promptActive.set(false);
        }
    }

...
final class VaultSnapshot {
    ...
    private void validate() throws VaultFailure {
        ...
        switch (phase) {
            ...
            case CANCEL_REQUESTED -> requireCancellation();
    ...
    private void requireCancellation() throws VaultFailure {
        boolean validSecretState = (secretKind == SecretKind.ENROLLMENT_CODE && installation == null)
            || secretKind == SecretKind.DEVICE_CREDENTIAL;
        require(
            validSecretState && !operationId.isEmpty() && !deviceId.isEmpty()
                && !flow.isEmpty() && expiresAtMillis > 0
                && removalOperationId.isEmpty() && blockedReason.isEmpty()
        );
    }
...
final class VaultValidation {
    ...
    static String flow(String value) throws VaultFailure {
        ...
        if (!clean.equals("enrollment") && !clean.equals("recovery") && !clean.equals("replacement")) {

...
final class RemovalCoordinator {
    ...
    RemovalView remove(String operationId, String deviceId) throws VaultFailure {
        ...
        if (!Boolean.TRUE.equals(state.get("active"))) throw new VaultFailure("manager_native_removal_conflict");
        ...
        if (!gate.confirm(operation, device)) throw new VaultFailure("manager_native_removal_cancelled");
        return engine.removeEnrollment(operation, device);
    }
}

...
final class AndroidRemovalAuthorizationGate implements RemovalAuthorizationGate {
    ...
    public boolean confirm(String operationId, String deviceId) throws VaultFailure {
        if (!promptActive.compareAndSet(false, true)) throw new VaultFailure("custodial_native_removal_confirmation_busy");
        Activity activity = activitySupplier.get();
        if (activity == null || activity.isFinishing() || activity.isDestroyed()) {
            promptActive.set(false);
            throw new VaultFailure("custodial_native_removal_confirmation_unavailable");
        }
        ...
        activity.runOnUiThread(() -> {
            ...
            AlertDialog shown = new AlertDialog.Builder(activity)
                ...
                .setMessage("Remove " + deviceId + " from Custodial access? Offline work stays on the phone, but server access stops until a manager enrolls it again.")
        ...
        try {
            if (!decisionReady.await(TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
                ...
                throw new VaultFailure("custodial_native_removal_confirmation_timeout");
            }
            return Boolean.TRUE.equals(decision.get());
        } catch (InterruptedException error) {
            Thread.currentThread().interrupt();
            throw new VaultFailure("custodial_native_removal_confirmation_interrupted", error);
        } finally {
            promptActive.set(false);
        }
    }

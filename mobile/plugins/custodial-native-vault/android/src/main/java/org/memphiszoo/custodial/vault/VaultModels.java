...
final class EncryptedSecret {
    ...
    EncryptedSecret(String ciphertext, String iv) throws VaultFailure {
        this.ciphertext = VaultValidation.bounded(ciphertext, 8, 16_384, "custodial_native_vault_corrupt");
        this.iv = VaultValidation.bounded(iv, 8, 1024, "custodial_native_vault_corrupt");
    }
...
final class InstallationBinding {
    ...
    InstallationBinding(
        ...
        this.enrolledAt = VaultValidation.timestamp(enrolledAt, "custodial_native_invalid_binding");
        ...
        if (!migratedFromCredentialOnlyState && this.enrollmentOperationId.isEmpty()) {
            throw new VaultFailure("custodial_native_invalid_binding");
        }
    }
...
final class EnrollmentMetadata {
    ...
    EnrollmentMetadata(
        ...
        this(credentialId, credentialExpiresAt, resumeExpiresAt, deviceName, employeeId, employeeName, "");
    }
    ...
    EnrollmentMetadata(
        ...
        this.credentialId = VaultValidation.safeText(credentialId, 160, "custodial_native_invalid_enrollment_response");
        this.credentialExpiresAt = optionalTimestamp(credentialExpiresAt);
        this.resumeExpiresAt = optionalTimestamp(resumeExpiresAt);
        this.deviceName = VaultValidation.safeText(deviceName, 200, "custodial_native_invalid_enrollment_response");
        this.employeeId = VaultValidation.safeText(employeeId, 160, "custodial_native_invalid_enrollment_response");
        this.employeeName = VaultValidation.safeText(employeeName, 200, "custodial_native_invalid_enrollment_response");
        this.recoveryId = VaultValidation.safeText(recoveryId, 160, "custodial_native_invalid_enrollment_response");
    }
    ...
    private static String optionalTimestamp(String value) throws VaultFailure {
        ...
        return clean.isEmpty() ? "" : VaultValidation.timestamp(clean, "custodial_native_invalid_enrollment_response");
    }

    long resumeExpiryMillis(long fallback) throws VaultFailure {
        if (resumeExpiresAt.isEmpty()) return fallback;
        return VaultTimestamps.epochMillis(resumeExpiresAt, "custodial_native_invalid_enrollment_response");
    }
    ...
    Map<String, Object> safeStateRecord(String operationId, String flow, String deviceId) {
        ...
        if (!employeeId.isEmpty() || !employeeName.isEmpty()) {
            ...
            if (!employeeId.isEmpty()) employee.put("id", employeeId);
            if (!employeeName.isEmpty()) employee.put("name", employeeName);
            result.put("employee", VaultCollections.copyMap(employee));
        }
        return VaultCollections.copyMap(result);
    }
    ...
    public boolean equals(Object other) {
        ...
        return credentialId.equals(value.credentialId)
            ...
            && employeeId.equals(value.employeeId)
            && employeeName.equals(value.employeeName)
            && recoveryId.equals(value.recoveryId);
    }

    @Override
    public int hashCode() {
        return Objects.hash(credentialId, credentialExpiresAt, resumeExpiresAt, deviceName, employeeId, employeeName, recoveryId);
    }
}
...
final class LegacyMaterial implements AutoCloseable {
    ...
    LegacyMaterial(char[] credential, InstallationBinding binding, String legacySeal) throws VaultFailure {
        if (credential == null || credential.length < 1 || credential.length > 4096) {
            throw new VaultFailure("custodial_native_legacy_vault_invalid");
        }
...
final class VaultSnapshot {
    ...
    private static VaultFailure corrupt() {
        return new VaultFailure("custodial_native_vault_corrupt");
    }
...
final class VaultValidation {
    private static final java.util.regex.Pattern DEVICE = java.util.regex.Pattern.compile("^KIOSK[_-]?(\\d{1,2})$", java.util.regex.Pattern.CASE_INSENSITIVE);
    ...
    static String deviceId(String value) throws VaultFailure {
        ...
        if (!match.matches()) throw new VaultFailure("custodial_native_invalid_device");
        int number = Integer.parseInt(match.group(1));
        if (number < 2 || number > 10) throw new VaultFailure("custodial_native_invalid_device");
        return String.format(java.util.Locale.ROOT, "KIOSK_%02d", number);
    }

    static String operationId(String value) throws VaultFailure {
        String clean = value == null ? "" : value.trim();
        if (!OPERATION.matcher(clean).matches()) throw new VaultFailure("custodial_native_invalid_operation");
    ...
    static String bindingSeal(String value) throws VaultFailure {
        String clean = value == null ? "" : value.trim();
        if (!SEAL.matcher(clean).matches()) throw new VaultFailure("custodial_native_invalid_binding");
        return clean;
    }
    ...
    static String reason(String value) throws VaultFailure {
        String clean = value == null ? "" : value.trim().toLowerCase(java.util.Locale.ROOT);
        if (!REASON.matcher(clean).matches()) throw new VaultFailure("custodial_native_vault_corrupt");
        return clean;
    }

...
final class WebViewInputPolicy {
    ...
    static void validateBodyBase64(String encoded) throws VaultFailure {
        ...
        if (length > MAX_ENCODED_BODY_CHARS) throw new VaultFailure("custodial_native_request_too_large");
        if ((length & 3) != 0) throw new VaultFailure("custodial_native_body_refused");
        int padding = 0;
        ...
        for (int index = 0; index < length - padding; index += 1) {
            ...
            boolean valid = (character >= 'A' && character <= 'Z')
                ...
                || character == '+'
                || character == '/';
            if (!valid) throw new VaultFailure("custodial_native_body_refused");
        }
        for (int index = length - padding; index < length; index += 1) {
            if (encoded.charAt(index) != '=') throw new VaultFailure("custodial_native_body_refused");
        }
        long decoded = ((long) length / 4L) * 3L - padding;
        if (decoded > MAX_DECODED_BODY_BYTES) throw new VaultFailure("custodial_native_request_too_large");
    }

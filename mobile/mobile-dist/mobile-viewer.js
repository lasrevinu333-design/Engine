(() => {
  ...
  // src/viewer/app.js
  var API = "https://memphis-zoo-mcp.onrender.com";
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      const cards = [
        ["Locations", data.locations_total],
        ["Current", data.locations_current],
        ["Due soon", data.locations_due_soon],
        ["Overdue", data.locations_overdue],
        ["In progress", data.locations_in_progress],
        ["Cleanings today", data.cleanings_completed_today]
      ];
  ...
  async function loadEvents() {
    ...
    try {
      ...
      eventList.innerHTML = rows.length ? rows.map((event) => `<article class="event"><strong>${esc(event.event_name)}</strong><div class="muted">${esc(event.event_date)}${event.start_time ? ` \xB7 ${esc(String(event.start_time).slice(0, 5))}` : ""}${event.display_location ? ` \xB7 ${esc(event.display_location)}` : ""}</div></article>`).join("") : '<p class="muted">No upcoming events are currently published.</p>';
  ...
  document.getElementById("feedback-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok || !payload?.ok) throw new Error(payload?.error || `HTTP ${response.status}`);
      event.target.reset();
      feedbackStatus.textContent = "Feedback accepted for review.";
  ...
  void Network2.addListener("networkStatusChange", ({ connected }) => {

...
async function mockBackend(context) {
  ...
  await context.route('https://memphis-zoo-mcp.onrender.com/**', async (route) => {
    ...
    if (url.pathname === '/version') {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, version: 'release-2026.07.24.custodial-v3.14' }) });
      return;
    }
    if (url.pathname === '/release-manifest') {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ release_id: 'release-2026.07.24.custodial-v3.14' }) });
      return;
    }

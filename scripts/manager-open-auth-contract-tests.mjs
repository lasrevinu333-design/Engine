...
assert.doesNotMatch(hub,/Inspection entry|Inspection score|Insights &amp; Inspections/i);
...
for (const label of ['My Schedule', 'Memphis Messenger', 'Upcoming Events', 'Program Feedback']) assert.match(custodialMobile,new RegExp(`>${label}<`));
assert.match(custodialMobile,/id="home-memphis"[^>]*class="memphisHome"[\s\S]*<strong>Ask Memphis<\/strong>/);
...
assert.match(liveWorkflow,/grep -Fq 'Work Statistics'/);
assert.match(liveWorkflow,/grep -Fq 'Custodial Work Statistics'/);
assert.match(liveWorkflow,/! grep -Eiq 'Inspection entry\|Inspection score\|Insights &amp; Inspections'/);
assert.doesNotMatch(liveWorkflow,/grep -Fq 'Insights &amp; Inspections'/);

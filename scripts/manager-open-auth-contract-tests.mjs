...
assert.doesNotMatch(hub+hubClient,/manager-codes\/consume/i);
...
assert.match(access,/Browser Enrollment Window/);
assert.match(access,/Generate New 48-Hour Passcode/);
assert.match(access,/Native App Enrollment/);
assert.match(access,/Generate App Code/);
...
assert.doesNotMatch(access,/Generate Personal Code|Generate PC Invite|Generate Phone Invite|ops_pairing_token/i);
...
assert.match(liveWorkflow,/ops\/shared-enrollment/);
assert.match(liveWorkflow,/consumeSharedEnrollmentPasscode/);
assert.match(liveWorkflow,/Generate New 48-Hour Passcode/);
...
assert.match(liveWorkflow,/! grep -Fq 'ops\/manager-codes\/consume'/);

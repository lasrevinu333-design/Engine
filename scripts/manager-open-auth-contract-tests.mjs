...
assert.match(auth,/ops\/manager-codes\/consume/);
...
assert.match(auth,/one-time manager code/i);
...
assert.match(full,/one-time manager code/i);
...
assert.match(hub,/OPS MANAGER ACCESS/);
assert.match(hub,/consumeOpsManagerCode/);
assert.doesNotMatch(hub,/Read-only manager link/);
assert.doesNotMatch(hub,/Ops Manager password|Manager password|Generate PC Invite|Copy Invite Link/i);

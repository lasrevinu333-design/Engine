...
function fixture({enrolled=false,reject=false}={}){
  ...
  const window={MemphisCustodialSecurity:security,MemphisUI:{resolveOpenScanSession:()=>({state:'none'})},
    ...
    MemphisMobile:{whenReady:async()=>{},resumePendingSecurityWorkflow:async()=>{calls.push('resume');},readCustodialHomeCache:()=>null,saveCustodialHomeCache:async()=>true,
      requestJson:async path=>{calls.push(path);if(reject)throw Object.assign(new Error('Not authorized'),{status:403});return {authenticated:true,canonical_device_id:'KIOSK_08',employee_name:'Fixture Assigned Custodian',employee_role:'CUSTODIAL_EMPLOYEE'};}},
  ...
  return {context,elements,calls,ready:()=>context.__startup,setAssigned:()=>{status={...status,state:'enrolled',deviceId:'KIOSK_08'};subscriber({...status});}};
}

...
assert.match(custodialHtml, /id="scan-location-qr"[^>]*class="btn"[^>]*>NFC Tag Unavailable</);
assert.doesNotMatch(custodialHtml, /id="scan-location-qr"[^>]*class="btn primary"/);
assert.ok(custodialHtml.indexOf('id="scan-location-qr"') > custodialHtml.indexOf('id="employee-more"'), 'QR fallback must remain subordinate in More, not a primary Assigned Areas action');

...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '50'/, 'Custodial recovery source must pin the protected Build 50 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 50/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '41'/, 'Custodial recovery source must pin the protected Build 41 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 41/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

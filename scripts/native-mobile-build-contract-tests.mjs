...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '49'/, 'Custodial recovery source must pin the protected Build 49 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 49/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

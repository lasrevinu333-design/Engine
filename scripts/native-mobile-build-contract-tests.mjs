...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '37'/, 'Custodial recovery source must pin the protected Build 37 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 37/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

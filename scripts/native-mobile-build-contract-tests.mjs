...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '42'/, 'Custodial recovery source must pin the protected Build 42 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 42/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

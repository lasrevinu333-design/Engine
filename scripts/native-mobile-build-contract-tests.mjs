...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '36'/, 'Custodial recovery source must pin the protected Build 36 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 36/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '46'/, 'Custodial recovery source must pin the protected Build 46 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 46/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

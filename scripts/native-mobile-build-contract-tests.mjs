...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '38'/, 'Custodial recovery source must pin the protected Build 38 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 38/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '45'/, 'Custodial recovery source must pin the protected Build 45 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 45/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

...
assert.match(custodialAndroid, /PROJECT_BUILD_NUMBER: '44'/, 'Custodial recovery source must pin the protected Build 44 package');
assert.equal(
...
assert.throws(
  ...
  /recovery source must emit versionCode 44/,
);
assert.throws(
  () => createCustodialAndroidReleaseAcceptance({
    ...releaseAcceptanceInput,

...
for (const [file, context] of secondaryPages) {
  ...
  assert.match(source, /href="\.\/memphis-ui\.css\?v=release-2026\.07\.24\.custodial-v3\.16"/, `${file} must load the shared design tokens`);
  assert.match(source, /src="\.\/memphis-ui\.js\?v=release-2026\.07\.24\.custodial-v3\.16"/, `${file} must load the shared interaction layer`);
...
assert.match(messages, /href="\.\/memphis-ui\.css\?v=release-2026\.07\.24\.custodial-v3\.16"/, 'Messenger must load the shared design tokens');
assert.match(messages, /src="\.\/memphis-ui\.js\?v=release-2026\.07\.24\.custodial-v3\.16"/, 'Messenger must load the shared interaction layer');
...
console.log(JSON.stringify({
  ...
  release_id: 'release-2026.07.24.custodial-v3.16',

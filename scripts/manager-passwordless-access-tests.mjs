...
assert.match(auth, /OPS_MANAGER_CODE_CONSUME_URL/);
assert.match(auth, /listOpsManagers/);
assert.match(auth, /createOpsManager/);
assert.match(auth, /createOpsManagerCode/);
assert.match(auth, /consumeOpsManagerCode/);
assert.match(auth, /revokeOpsManagerCode/);
...
assert.match(managerAccess, /Change Manager Role/);
assert.match(managerAccess, /Revoke All Manager Devices/);
assert.match(managerAccess, /Disable Manager/);
assert.match(managerAccess, /createOpsManagerCode/);
assert.match(managerAccess, /revokeOpsManagerCode/);
...
assert.match(managerAccess, /ttl_seconds:900/);
assert.match(managerAccess, /max_attempts:5/);
assert.doesNotMatch(managerAccess, /Generate PC Invite|Generate Phone Invite|Additional Device Invite|Copy Invite Link|Display Invite QR/);
assert.doesNotMatch(managerAccess, /QRCode\.toCanvas|qrcode\.min\.js|ops_pairing_token|enrollment_url/);
...
assert.match(startPage, /OPS MANAGER ACCESS/);
assert.match(startPage, /One-time manager code/);
assert.match(startPage, /consumeOpsManagerCode/);
assert.match(startPage, /Codes expire after 15 minutes and work once/);
assert.doesNotMatch(startPage, /Open a one-click Ops Manager invitation/);

...
contains('lock exposes the enrolled employee identity node', 'id="lock-employee-name"');
contains('Home exposes the enrolled employee identity node', 'id="home-employee-name"');
contains('employee identity comes from the protected current profile', 'window.MemphisMobile?.readCustodialHomeCache?.()?.profile');
contains('employee identity refreshes from device authority', 'refreshCredentialStatus?.({force:true})');
contains('employee identity requires authenticated authority', "value?.authenticated===true&&applyEmployeeIdentity(value)");
doesNotContain('lock must not retain generic Memphis Zoo identity', '<div class="lockAssigned">Memphis Zoo</div>');

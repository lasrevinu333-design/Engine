(function(){
  ...
  async function consumeSharedEnrollmentPasscode(options={}){
    ...
    if(!code)throw new Error('Enter the eight-digit shared enrollment passcode.');
    ...
    try{
      return await parseSessionResponse(await fetch(`${OPS_SHARED_ENROLLMENT_URL}/consume`,{
        ...
        body:JSON.stringify({code,device_id:getDeviceId(),device_label:label})
      }));
    }finally{
  ...
  async function disableSharedEnrollmentWindow(windowId,reason='disabled_by_custodial_manager'){
    ...
    if(!/^[0-9a-f-]{36}$/i.test(normalizedWindowId))throw new Error('A valid enrollment window ID is required.');
    const headers=await opsManagerAuthHeaders();headers['Content-Type']='application/json';
    const response=await fetch(`${OPS_SHARED_ENROLLMENT_URL}/${encodeURIComponent(normalizedWindowId)}/disable`,{
      method:'POST',cache:'no-store',credentials:'include',headers,body:JSON.stringify({reason})
    });
    const payload=await response.json().catch(()=>null);
    if(!response.ok||!payload?.ok){
      const error=new Error(payload?.error||`Enrollment disable failed: HTTP ${response.status}`);
      error.status=response.status;error.payload=payload;throw error;
    }
    return payload.data;
  }
  ...
  async function requireOpsManagerSession(options={}){
    ...
    try{
      ...
      if(options.throwOnFailure===true)throw new Error('This browser is not trusted for Operations Leadership access. Enter the shared 48-hour enrollment passcode on the Hub entry page.');
      return null;
    }catch(error){
      if(redirect&&!/\/(?:start_page1|ops-manager-hub)\.html$/i.test(window.location.pathname||''))redirectToManagerHub(requested);
